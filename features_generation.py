...
def calculate_deviations(list_of_df, dict_values = {188: [50,150], 200: [20, 55]}):
    """ Функция рассчитывает статистики и отклонения от них каждой точки в каждом столбце в списке DataFrame.
    Параметры: 
    ...
    :return: Список списков, где каждый внутренний список содержит: 
    - DataFrame с отклонениями от рассчитанных статистик для каждой точки каждого столбца, 
